...
import org.ict.classifier.cca.DecisionSerge;
import org.ict.classifier.cca.centerCell;
import org.ict.classifier.cca.ECCAClusterizator;
import org.ict.classifier.cca.Mesh;
...
public class PECCAProcesslet implements Processlet {

  private static final Logger LOG = LoggerFactory.getLogger( PECCAProcesslet.class );
  ...
  private static final String M_MIN = "mMin";
  private static final String M_COL = "mCol";
  private static final String STOP_CRIT = "stopCrit";
  ...
  public void process( ProcessletInputs in, ProcessletOutputs out, ProcessletExecutionInfo info ) throws ProcessletException {
    ...
    LOG.trace( "BEGIN PECCAProcesslet#execute(), context: " + OGCFrontController.getContext() );
    ...
    LOG.trace( "END PECCAProcesslet#execute()" );
  }

  public void destroy() {
    LOG.debug( "PECCAProcesslet#destroy() called" );
  }

  public void init() {
    LOG.debug( "PECCAProcesslet#init() called" );
  }
  ...
  private OutputBundle doProcess(InputBundle inputBundle, ProcessletExecutionInfo info) {
    ...
    int mMin = inputBundle.mMin;                  
    double stopCrit = inputBundle.stopCrit;      
    double densityThreshold = inputBundle.densityThreshold;        
    int mCol = inputBundle.mCol;   
    
    if ((mMin <= 0) || (mMin+mCol>256))
    	mMin=30;
    if ((stopCrit < 0) || (stopCrit > 1))
    	stopCrit=0.8;
    if (densityThreshold < 0)
    	densityThreshold=0.9;
    if ((mCol < 0) || (mMin+mCol>256))
    	mCol=15;
	...
	ECCAClusterizator cca = new ECCAClusterizator();
  ...
  private InputBundle parseInputs(ProcessletInputs inputs) throws ProcessletException {
    try {
      int mMin;
        LiteralInput inputMMin = (LiteralInput) inputs.getParameter(M_MIN);
        mMin = Integer.parseInt( inputMMin.getValue() );
        
      int mCol;
        LiteralInput inputMCol = (LiteralInput) inputs.getParameter(M_COL);
        mCol = Integer.parseInt( inputMCol.getValue() );
      
      double stopCrit;
          LiteralInput inputStopCrit = (LiteralInput) inputs.getParameter(STOP_CRIT);
          stopCrit = Double.parseDouble( inputStopCrit.getValue() );
...
//        opts.add(RasterIOOptions.OPT_FORMAT, mimeType.getSubType());
      ...
      return new InputBundle(mMin, mCol, stopCrit, densityThreshold, raster);
...
//      ImageIO.write(outputBundle.image,  "png", new File("/tmp/wps/result.png") );
  ...
  private static class InputBundle {
	  final int mMin;
	    final int mCol;
	    final double stopCrit;
	    final double densityThreshold;
	    final BufferedImage raster;
	    
	    private InputBundle(int mMin, int mCol, double stopCrit, double densityThreshold, BufferedImage raster) {
	      assert raster != null;
	      this.mMin = mMin;
	      this.mCol = mCol;
