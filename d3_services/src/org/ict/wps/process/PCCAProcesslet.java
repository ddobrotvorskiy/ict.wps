...
public class PCCAProcesslet implements Processlet {

  private static final Logger LOG = LoggerFactory.getLogger( PCCAProcesslet.class );
  ...
  private static final String EMPTY_THREASHHOLD = "EmptyThreshold";
  private static final String BACKGROUND_PERCENT = "BackgroundPercent";
  private static final String DENSITY_THREASHHOLD = "DensityThreshold";



  public void process( ProcessletInputs in, ProcessletOutputs out, ProcessletExecutionInfo info ) throws ProcessletException {

    LOG.trace( "BEGIN PCCAProcesslet#execute(), context: " + OGCFrontController.getContext() );
    ...
    info.setSucceededMessage("PClassification succeeded");

    LOG.trace( "END PCCAProcesslet#execute()" );
  }

  public void destroy() {
    LOG.debug( "PCCAProcesslet#destroy() called" );
  }

  public void init() {
    LOG.debug( "PCCAProcesslet#init() called" );
  }
  ...
  private OutputBundle doProcess(InputBundle inputBundle, ProcessletExecutionInfo info) {
    ...
    int cellsize = inputBundle.cellsize;                  
    double backgroundpercent = inputBundle.backgroundpercent;      
    double densityThreshold = inputBundle.densityThreshold;        
    int emptyThreshold = inputBundle.emptyThreshold;   
    
    if ((cellsize <= 0) || (cellsize>256))
    	cellsize=30;
    if ((backgroundpercent < 0) || (backgroundpercent > 1))
    	backgroundpercent=0;
    ...
    param.backgroundpercent = backgroundpercent;      
    param.densityThreshold = densityThreshold;        
    param.emptyThreshold = emptyThreshold;         
    param.forCCA = true;
  ...
  private InputBundle parseInputs(ProcessletInputs inputs) throws ProcessletException {
    try {
      int cellsize;
        LiteralInput inputCellsize = (LiteralInput) inputs.getParameter(CELL_SIZE);
        cellsize = Integer.parseInt( inputCellsize.getValue() );

      int emptyThreshold;
        LiteralInput inputEmptyThreshold = (LiteralInput) inputs.getParameter(EMPTY_THREASHHOLD);
        emptyThreshold = Integer.parseInt( inputEmptyThreshold.getValue() );
      
      double backgroundpercent;
          LiteralInput inputBackgroundpercent = (LiteralInput) inputs.getParameter(BACKGROUND_PERCENT);
          backgroundpercent = Double.parseDouble( inputBackgroundpercent.getValue() );
      
      double densityThreshold;
          LiteralInput inputDensityThresholdt = (LiteralInput) inputs.getParameter(DENSITY_THREASHHOLD);
          densityThreshold = Double.parseDouble( inputDensityThresholdt.getValue() );
...
//        opts.add(RasterIOOptions.OPT_FORMAT, mimeType.getSubType());
      ...
      return new InputBundle(cellsize, emptyThreshold, backgroundpercent, densityThreshold, raster);
...
//      ImageIO.write(outputBundle.image,  "png", new File("/tmp/wps/result.png") );
  ...
  private static class InputBundle {
	  final int cellsize;
	    final int emptyThreshold;
	    final double backgroundpercent;
	    final double densityThreshold;
	    final BufferedImage raster;

	    private InputBundle(int cellsize, int emptyThreshold, double backgroundpercent, double densityThreshold, BufferedImage raster) {
	      assert raster != null;
