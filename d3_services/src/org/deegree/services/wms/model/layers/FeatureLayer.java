//$HeadURL: http://wald.intevation.org/svn/deegree/deegree3/services/trunk/src/org/deegree/services/wms/model/layers/FeatureLayer.java $
/*----------------------------------------------------------------------------
 ...
 * 
 * @version $Revision: 21646 $, $Date: 2009-12-23 16:11:57 +0600 (Срд, 23 Дек 2009) $
 */
        ...
        try {
            ...
            if ( featureType == null ) {
                List<Query> queries = map( datastore.getSchema().getFeatureTypes( false, false ),
                                           new Mapper<Query, FeatureType>() {
                                               public Query apply( FeatureType u ) {
                                                   return new Query( u.getName(), fi.getClickBox( 0 ),
                                                                     dimFilter == null ? null : dimFilter.first,
                                                                     fi.returnGeometries(), -1 );
                                               }
                                           } );
                col = datastore.query( queries.toArray( new Query[queries.size()] ) ).toCollection();
            } else {
                Query query = new Query( featureType, fi.getClickBox( 0 ), dimFilter == null ? null : dimFilter.first,
                                         fi.returnGeometries(), -1 );
